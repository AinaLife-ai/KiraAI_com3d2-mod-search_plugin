...
async def _install_chromium() -> bool:
    ...
    try:
        ...
        logger.info("[com3d2-search] Chromium 下载完成")
    except Exception as e:
        logger.warning(f"[com3d2-search] Chromium 自动下载失败: {e}")
        return False
    # 验证安装
    try:
        ...
        async with async_playwright() as p:
            ...
            await browser.close()
        logger.info("[com3d2-search] Chromium 安装验证通过")
        return True
    except Exception as e:
        logger.warning(f"[com3d2-search] Chromium 安装后仍无法启动: {e}")
        return False
...
class BrowserManager:
    ...
    async def initialize(self):
        try:
            ...
            logger.warning(f"[com3d2-search] playwright 未安装，渲染图模式不可用: {e}")
            self._ready.set()
            return
        try:
            # 1) 系统默认浏览器
            for channel, display in _BROWSER_CANDIDATES:
                try:
                    async with async_playwright() as p:
                        ...
                        await browser.close()
                    logger.info(f"[com3d2-search] 检测到系统浏览器: {display}")
            ...
            try:
                async with async_playwright() as p:
                    ...
                    await browser.close()
                self._ok = True
                logger.info("[com3d2-search] 使用 Playwright 内置 Chromium")
                return
            except Exception:
                # 3) 自动下载依赖
                logger.info("[com3d2-search] 未检测到可用浏览器，自动下载内置 Chromium...")
                if await _install_chromium():
                    self._ok = True
        except Exception as e:
            logger.warning(f"[com3d2-search] 浏览器初始化失败，渲染图模式不可用: {e}")
        finally:
            self._ready.set()


async def render_html(html: str, output_path: str, browser: BrowserManager) -> str:
